...
const BookDescription = () => {
  ...
  const getDetail = async () => {
    const res = await axios.get(`http://localhost:8000/audiobooks/${param.id}`);
    setDetail(res.data);
    if (
  ...
  const submitHandler = async(e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:8000/audiobooks/${param.id}/reviews`, { name:reviewer,rating:rating,comment:comment }).then(()=>{
